...
- [ ] Understand how Kotlin compiles to JVM bytecode and what `.class` files are produced
- [ ] `val` vs `var` — immutability at reference level vs mutation; understand why `val` does not guarantee deep immutability
- [ ] Primitive types vs Kotlin's type system (`Int`, `Long`, `Double`, `Boolean`, `Char`) and how they map to JVM primitives vs boxed types
- [ ] String templates and multi-line strings (`trimIndent`, `trimMargin`)
- [ ] Type inference — when Kotlin infers types and when you must annotate explicitly
- [ ] Nullable types: `?`, `!!`, `?.`, `?:`, `let`, `also`, `run`, `apply`, `with`
- [ ] Smart casts: how the compiler tracks nullability and type after checks
...
- [ ] Named parameters and default argument values
...
- [ ] Higher-order functions — passing functions as parameters, returning functions
- [ ] Lambdas with receivers — the basis for DSLs, how `this` is captured
- [ ] `it` vs named lambda parameters — readability rules
- [ ] Function types: `(Int, String) -> Boolean`, `suspend () -> Unit`

#### 0.1.3 OOP

- [ ] Classes, constructors (primary/secondary), `init` blocks
- [ ] `data class` — `copy()`, `equals()`, `hashCode()`, `toString()`, destructuring, component functions
- [ ] `sealed class` vs `enum class` — when to use which for state modeling
- [ ] `object` declarations — singletons, companion objects, anonymous objects
- [ ] `interface` vs `abstract class` — default method implementations, multiple inheritance
- [ ] Delegation: `by` keyword, implementing interface via a delegate, property delegation
...
- [ ] `List`, `MutableList`, `Set`, `Map`, `Sequence` — differences and when to prefer each
...
- [ ] Range operators: `..`, `until`, `downTo`, `step`
...
- [ ] Android OS layers: Linux kernel → Hardware Abstraction Layer (HAL) → Android Runtime → Framework → Applications
...
- [ ] ART (Android Runtime) vs Dalvik — ART uses AOT (Ahead-Of-Time) compilation; Dalvik used JIT
...
- [ ] `onSaveInstanceState(Bundle)` vs `ViewModel` — what gets saved to Bundle (small, serializable data) vs ViewModel (in-memory during config change)
...
- [ ] Task and back stack: what a Task is, how `FLAG_ACTIVITY_NEW_TASK` and `FLAG_ACTIVITY_CLEAR_TOP` work
- [ ] Launch modes: `standard`, `singleTop`, `singleTask`, `singleInstance` — behavioral differences and correct usage
...
- [ ] `viewLifecycleOwner` vs `this` (fragment) as LifecycleOwner — always use `viewLifecycleOwner` for UI-related observers
- [ ] Fragment arguments: always use `newInstance()` factory pattern with Bundle arguments; never constructor parameters
...
- [ ] Started service: `startService()` / `stopSelf()` — `START_STICKY`, `START_NOT_STICKY`, `START_REDELIVER_INTENT` return values and their meanings
- [ ] Bound service: `bindService()`, `ServiceConnection`, `IBinder` — connection lifecycle
- [ ] Foreground service: mandatory `startForeground()` with notification, types (`dataSync`, `mediaPlayback`, `location`, etc.) required since Android 14
...
- [ ] Manifest-declared vs dynamically registered receivers — which still work for implicit broadcasts in Android 8+
- [ ] `LocalBroadcastManager` (deprecated) → use `Flow` or `LiveData` instead
- [ ] Common system broadcasts: `ACTION_BOOT_COMPLETED`, `ACTION_BATTERY_CHANGED`, `CONNECTIVITY_ACTION`, `ACTION_SCREEN_ON`
...
- [ ] Explicit intent: specify component class directly — used for internal navigation
- [ ] Implicit intent: specify action + category + data — resolved by system to matching app
- [ ] Intent flags: `FLAG_ACTIVITY_NEW_TASK`, `FLAG_ACTIVITY_CLEAR_TASK`, `FLAG_ACTIVITY_SINGLE_TOP`, `FLAG_GRANT_READ_URI_PERMISSION`
- [ ] `PendingIntent` — snapshot of an intent for later execution by another process (notifications, widgets, alarms)
- [ ] Intent filters in manifest: `<action>`, `<category>`, `<data>` and how they determine app selection
- [ ] Deep links: `<data android:scheme="https" android:host="...">`, `App Links` (verified), `Custom schemes`
...
- [ ] `ConstraintLayout` — chains (horizontal/vertical), barriers, groups, `Guidelines`, `ConstraintSet` for programmatic changes
...
- [ ] Gap buffer / positional memoization — how `remember` stores state in the slot table by call site
...
- [ ] Lambda capture stability — unstable lambdas cause recomposition; use `remember { lambda }` or `rememberUpdatedState`
...
- [ ] Strong skipping mode (experimental as of 2024) — allows skipping even unstable params if references equal
- [ ] Layout Inspector → Compose tab — shows recomposition counts per composable in Android Studio
...
- [ ] `ViewModel` + `StateFlow` → `collectAsState()` / `collectAsStateWithLifecycle()` as the standard pattern
- [ ] `collectAsStateWithLifecycle()` — stops collecting when UI is in background (safer than `collectAsState`)
- [ ] `derivedStateOf {}` — memoizes a computed value, only recomposes when output changes (not when input changes but output does not)
- [ ] `snapshotFlow {}` — converts `State<T>` into a `Flow<T>`; does NOT cause recomposition

#### 3.2.4 Side Effects

- [ ] `LaunchedEffect(key)` — starts a coroutine scoped to composition; re-launched when key changes; cancelled on leave
- [ ] `DisposableEffect(key)` — setup + teardown (onDispose); for registering/unregistering listeners
- [ ] `SideEffect {}` — runs after every successful recomposition; for syncing Compose state to non-Compose code
- [ ] `rememberUpdatedState(value)` — captures latest value inside a long-running effect without restarting it
- [ ] `rememberCoroutineScope()` — get a `CoroutineScope` tied to composition; use for user-initiated actions
...
- [ ] `Arrangement` — `SpaceBetween`, `SpaceAround`, `SpaceEvenly`, `Center`, `Start`, `End`, `Top`, `Bottom`
...
- [ ] `SubcomposeLayout` — lazy layout that measures children before knowing their sizes (used by `LazyColumn` internally)
...
- [ ] `LazyListState` — `rememberLazyListState()`, `firstVisibleItemIndex`, `scrollToItem()`, `animateScrollToItem()`
...
- [ ] `CompositionLocal` — ambient values propagated implicitly down the tree; `LocalContext`, `LocalDensity`, `LocalConfiguration`
...
- [ ] Expose UI state as a single `UiState` sealed class or data class from ViewModel, not multiple streams
- [ ] `StateFlow` for state, `SharedFlow` (or `Channel`) for one-time events (navigation, snackbars)
...
- [ ] `UseCase` / `Interactor` — encapsulate a single business operation, called by ViewModel
...
- [ ] Module types: `:app`, `:feature:home`, `:feature:profile`, `:core:ui`, `:core:data`, `:core:network`, `:core:testing`
...
- [ ] Navigation between feature modules — using a shared navigation graph or a navigation abstraction
...
- [ ] `@Binds` — abstract method binding interface to implementation; compile-time efficient, no code generated
...
- [ ] `@Qualifier` — distinguish between two `@Provides` methods that return the same type (`@Named` is the simple built-in)
- [ ] `@HiltViewModel` — inject ViewModel with Hilt; `hiltViewModel()` in Compose
- [ ] Assisted injection (`@AssistedInject`, `@AssistedFactory`) — inject runtime parameters alongside DI parameters
...
- [ ] Component hierarchy: `SingletonComponent` → `ActivityRetainedComponent` → `ViewModelComponent` → `ActivityComponent` → `FragmentComponent` → `ViewComponent`

### Chapter 5.2 — Dagger 2 Internals (Understanding the Foundation)
...
- [ ] `RoomDatabase.Builder` — `addMigrations()`, `fallbackToDestructiveMigration()`, `allowMainThreadQueries()` (tests only)
...
- [ ] Single source of truth: Room is the source of truth; network responses are written to Room, UI reads from Room
...
- [ ] `Application Interceptor` vs `Network Interceptor` — application interceptors see redirects and retries as one call
...
- [ ] `CertificatePinner` — pin specific certificates for endpoints (certificate pinning)
...
- [ ] **Moshi** — Kotlin-first; `@JsonClass(generateAdapter = true)`, `@Json(name = "field_name")`, `Moshi.Builder().addLast(KotlinJsonAdapterFactory())`
...
- [ ] Token storage: `EncryptedSharedPreferences` or `Keystore` — NEVER plain SharedPreferences or plain files
- [ ] Token refresh flow: intercept 401 → acquire new token (with mutex to prevent parallel refreshes) → retry request
...
- [ ] Sealed class `NetworkResult<T>` — `Success(data)`, `Error(code, message)`, `Exception(e)` — wrap all API calls
...
- [ ] `Constraints.Builder()` — `requiresNetworkType(NetworkType.CONNECTED)`, `requiresBatteryNotLow()`, `requiresStorageNotLow()`, `requiresCharging()`, `requiresDeviceIdle()`
...
- [ ] Unique work — `enqueueUniqueWork()` and `enqueueUniquePeriodicWork()` with `ExistingWorkPolicy` (`KEEP`, `REPLACE`, `APPEND`, `APPEND_OR_REPLACE`)
...
- [ ] WorkManager vs `AlarmManager` — WorkManager is battery-aware and deferred; AlarmManager is exact
- [ ] WorkManager vs `JobScheduler` — WorkManager wraps JobScheduler and adds chaining/constraints API
...
- [ ] Common leak patterns: Activity/Fragment context held in static fields, anonymous inner classes (e.g., Handler, Runnable), non-cancelled coroutines, unremoved listeners
...
- [ ] Cold start vs warm start vs hot start — cold is most expensive (Zygote fork + DEX load + Application.onCreate + first Activity)
...
- [ ] **MockK** — Kotlin-native mocking: `mockk<T>()`, `every { } returns`, `coEvery { } returns` for coroutines, `verify { }`, `coVerify { }`
- [ ] `relaxed = true` parameter — auto-stubs all calls, useful for tests focused on only a few interactions
...
- [ ] **Mockito** — Java mocking, less idiomatic for Kotlin; `mock()`, `when().thenReturn()`, `verify()`
...
- [ ] Injecting dispatchers — constructor-inject `CoroutineDispatcher` and replace with `TestDispatcher` in tests
- [ ] Testing `Flow` emissions — use `turbine` library: `flow.test { assertThat(awaitItem()) }`, `cancelAndConsumeRemainingEvents()`
...
- [ ] Fake vs Mock — prefer `Fake` implementations for repositories (real logic, fake data source) over mocks for complex tests
...
- [ ] `MockWebServer` (OkHttp) — fake HTTP server for integration tests; `enqueueResponse()`, `takeRequest()`
...
- [ ] Cleartext traffic — `android:usesCleartextTraffic="false"` in manifest; override per domain in network security config
...
- [ ] Permanently denied detection — if false AND `checkSelfPermission` is DENIED, user has permanently denied
...
- [ ] Android 13+ media permissions: `READ_MEDIA_IMAGES`, `READ_MEDIA_VIDEO`, `READ_MEDIA_AUDIO` replace `READ_EXTERNAL_STORAGE`
- [ ] Android 14: `READ_MEDIA_VISUAL_USER_SELECTED` — partial media access

### Chapter 11.4 — Data Privacy & Scoped Storage

- [ ] Scoped storage (Android 10+) — apps cannot access arbitrary files; use `MediaStore` for shared media, SAF for user-chosen files
- [ ] `Storage Access Framework (SAF)` — `ACTION_OPEN_DOCUMENT`, `ACTION_CREATE_DOCUMENT`, `takePersistableUriPermission()`
- [ ] `MediaStore` API — query, insert, update media; requires appropriate permissions
...
- [ ] R8 — full-mode obfuscation, shrinking, and optimization; successor to ProGuard
...
- [ ] `android:debuggable="false"` — must be false in release builds (AGP does this automatically)
- [ ] Detecting root — `RootBeer` library for basic root detection
- [ ] Preventing backup of sensitive data — `android:allowBackup="false"` or `android:fullBackupContent`
- [ ] SSL/TLS version enforcement — `SSLContext.getInstance("TLS")` enforces minimum TLS 1.2 on modern Android
...
- [ ] `android {}` block: `compileSdk`, `minSdk`, `targetSdk`, `defaultConfig`, `buildTypes`, `productFlavors`, `buildFeatures`
...
- [ ] `flavorDimensions` — combine multiple dimensions (e.g., `environment` × `tier`)
- [ ] Build variant = buildType + productFlavor combination
- [ ] `versionCode` and `versionName` — automating with CI build number
- [ ] Dependency configurations: `implementation`, `api`, `testImplementation`, `androidTestImplementation`, `debugImplementation`, `ksp`, `kapt`
- [ ] Gradle caching — local build cache, remote build cache in CI
- [ ] Configuration cache — `--configuration-cache` flag; Gradle re-uses task graph without re-running configuration phase
- [ ] `buildSrc` for shared build logic (being replaced by convention plugins)
...
- [ ] `proguard-rules.pro` common rules: `-keep class * implements android.os.Parcelable`, keep model classes used with Gson, keep JNI classes
...
- [ ] AAB vs APK — AAB is required for Play Store; Play splits into APKs per device config
...
- [ ] `Firebase Crashlytics` — add `FirebaseCrashlytics.getInstance()`, `recordException()`, custom keys
...
- [ ] `contentDescription` — required for all images and icons; must be meaningful, not "image"
...
- [ ] `FusedLocationProviderClient` (Google Play Services) — `getLastLocation()`, `requestLocationUpdates()`
- [ ] `LocationRequest.Builder` — `priority` (`PRIORITY_HIGH_ACCURACY`, `PRIORITY_BALANCED_POWER_ACCURACY`), `intervalMillis`, `minUpdateIntervalMillis`
...
- [ ] Geofencing: `GeofencingClient.addGeofences()`, `GeofenceRequest`, `GeofencingEvent` in receiver
...
- [ ] KMP + Compose Multiplatform (experimental) — share UI across Android, iOS, Desktop

### Chapter 15.2 — On-Device ML

- [ ] `ML Kit` — Google's on-device ML SDK; text recognition, face detection, barcode scanning, language detection
- [ ] `TensorFlow Lite` — run custom `.tflite` models; interpreter API
- [ ] `LiteRT (formerly TFLite)` — Google's rebranding of TFLite as of 2024
...
- [ ] Window size classes — `WindowWidthSizeClass.Compact/Medium/Expanded`, `WindowHeightSizeClass`
...
- [ ] **Chat / Real-Time App** — WebSocket (OkHttp or Socket.IO), FCM push notifications, Room message caching, Compose UI, multi-module (`:feature:chat`, `:core:network`, `:core:data`), unit tests + Compose UI tests, CI (GitHub Actions)
