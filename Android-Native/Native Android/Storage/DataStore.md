...
### _From History to System Internals to Production Patterns_
...
By default, `dataStore.data` emits every time any preference changes. But your `.map {}` extracts only one value. If you change a _different_ key, your mapped flow won't re-emit.
...
_This guide covers DataStore 1.1.1 — the version in your WallStreet project (libs.versions.toml: `datastorePreferences = "1.1.1"`)._