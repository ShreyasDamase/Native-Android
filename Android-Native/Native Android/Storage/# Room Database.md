...
## 7.6 Room in Kotlin Multiplatform (KMP)

Starting with **Room 2.7.0+**, Room officially supports Kotlin Multiplatform (KMP). You can define your entities, DAOs, and database classes once inside `commonMain` and access them across Android, iOS, and Desktop.

### 1. Project Dependencies (build.gradle.kts)
Apply the Room compiler (KSP) to your targets and add the runtime dependencies:
```kotlin
plugins {
...
kotlin {
    ...
    sourceSets {
        commonMain.dependencies {
            ...
            implementation("androidx.sqlite:sqlite-bundled:2.5.0") // Bundled SQLite driver for iOS/Desktop
        }
    }
}

dependencies {
    // Compiler applied globally via KSP to handle code generation
    add("kspCommonMainMetadata", "androidx.room:room-compiler:2.7.0")
}
```

### 2. The Shared Database in commonMain
Your database and entities are identical to normal Android Room, but reside in `commonMain/kotlin/`:
```kotlin
// commonMain/src/AppDatabase.kt
@Database(entities = [UserEntity::class], version = 1)
@ConstructServices(AppDatabaseConstructor::class) // Required constructor helper for KMP
abstract class AppDatabase : RoomDatabase() {
    abstract fun userDao(): UserDao
}

// Expect function to get database builders from platforms
expect fun getDatabaseBuilder(context: PlatformContext): RoomDatabase.Builder<AppDatabase>
```

### 3. Platform Builders (expect/actual)

Since file paths differ across Android and iOS, you construct the SQLite builder on the native sides:

#### Android implementation (`androidMain`):
```kotlin
// androidMain/src/DatabaseBuilder.android.kt
...
```kotlin
// iosMain/src/DatabaseBuilder.ios.kt
import platform.Foundation.NSHomeDirectory

actual fun getDatabaseBuilder(context: PlatformContext): RoomDatabase.Builder<AppDatabase> {
    val dbFile = NSHomeDirectory() + "/Documents/app_database.db"
    return Room.databaseBuilder<AppDatabase>(
        name = dbFile,
        factory = { AppDatabase::class.instantiateImpl() } // Generated by Room compiler
    ).setDriver(BundledSQLiteDriver()) // Forces iOS to use bundled SQLite
}
```
