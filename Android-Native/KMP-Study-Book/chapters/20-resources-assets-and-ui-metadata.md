...
## 20.5 Modern KMP: Stable Multiplatform Resources API

Compose Multiplatform 1.6+ introduced a **Stable Multiplatform Resources API** that replaces complex platform-specific resource mappings.

*   **Shared Assets:** Strings, fonts, vectors, and raw files are stored inside `commonMain/composeResources/`.
*   **The `Res` Accessor:** The build system generates a type-safe `Res` configuration object. You load assets in shared UI scripts using:
    *   `stringResource(Res.string.app_name)`
    *   `painterResource(Res.drawable.app_logo)`
*   **Under the Hood:** During builds, the KMP toolchain compiles these files into native Android XML formats and iOS asset bundles automatically.
