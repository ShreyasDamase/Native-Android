...
## 13.6 Modern UI Architecture: Common ViewModels & Type-Safe Navigation

Modern KMP projects align UI architecture with Jetpack Compose standard conventions:

*   **Common Jetpack ViewModels:** You can declare Jetpack ViewModels directly in `commonMain` using `androidx.lifecycle:lifecycle-viewmodel-compose`. You no longer need to write platform-specific ViewModel wrappers for Android and iOS.
*   **Type-Safe Navigation:** You define navigation routes in `commonMain` using `@Serializable` classes or objects. This prevents route parsing errors and enforces compile-time type checks across platforms.
