...
## 3.9 Kotlin 2.0 Unification & Version Alignment

Starting with Kotlin 2.0.0, the Compose Compiler is hosted directly inside the main Kotlin compiler repository. 

*   **Before Kotlin 2.0:** Developers had to manually map compatible Compose Compiler versions to Kotlin versions in `libs.versions.toml`.
*   **After Kotlin 2.0:** You apply the new Kotlin Compose Compiler Gradle plugin: `kotlin("plugin.compose")` (or `org.jetbrains.kotlin.plugin.compose`). The build system matches the compiler plugin version to your active Kotlin version automatically, resolving mismatch issues.
