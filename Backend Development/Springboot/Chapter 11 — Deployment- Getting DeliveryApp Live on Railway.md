...
For DeliveryApp you need four Railway services:
...
```bash
# Build the image
docker build -t deliveryapp-api .

# Run it locally with test environment variables
docker run -p 8080:8080 \
  -e SPRING_PROFILES_ACTIVE=prod \
  -e DB_URL=jdbc:postgresql://host.docker.internal:5432/deliveryapp_dev \
...
Click **New Project** → **Empty Project**. Name it `deliveryapp`.
...
- Select your DeliveryApp backend repository
...
# ── CORS ──────────────────────────────────────────────────────────
CORS_ALLOWED_ORIGINS=https://deliveryapp.com,https://www.deliveryapp.com
...
```kotlin
// src/main/kotlin/com/example/deliveryapp/health/DatabaseHealthIndicator.kt

package com.example.deliveryapp.health
...
INFO  c.e.h.DeliveryAppApplication - Started DeliveryAppApplication in 8.234 seconds
...
Railway gives you a URL like `deliveryapp-api.up.railway.app`. For production, point your custom domain at it:

**Step 1 — Add domain in Railway**

- Click your service → **Settings** → **Networking**
- Click **Generate Domain** for the Railway subdomain
- Click **Custom Domain** → enter `api.deliveryapp.com`
...
Type: CNAME
Name: api
Value: deliveryapp-api.up.railway.app
TTL: 3600
```

**Step 3 — Update your app**

Update `CORS_ALLOWED_ORIGINS` to include your production domains. Update `CLERK_JWKS_URL` if you have a custom Clerk domain. Update the Android app's base URL to `https://api.deliveryapp.com`.
...
# CORS
CORS_ALLOWED_ORIGINS=https://deliveryapp.com
...
BASE=https://deliveryapp-api.up.railway.app/api
...
curl -X POST https://deliveryapp-api.up.railway.app/api/admin/companies \
...
```bash
curl -X POST https://deliveryapp-api.up.railway.app/api/admin/crawler/trigger \
  -H "Authorization: Bearer $ADMIN_TOKEN"
...
## 11.17 DeliveryApp Connection — What You Built in Chapter 11

By the end of Chapter 11, DeliveryApp's backend is live:
...
- **Custom domain** — `api.deliveryapp.com` pointing to Railway
...
Open Railway logs. Watch Flyway run. Watch the app start. Look for `Started DeliveryAppApplication in X seconds`.
...
```
DeliveryApp Backend — fully deployed
│
├── API Layer (Chapter 4)
...
## Book-Aligned Corrections: Deployment

The book adds several Spring Boot deployment ideas beyond "upload the app":

- `bootJar` produces a runnable JVM artifact.
- Docker image creation can use Spring Boot build image support.
- Spring Boot 3 supports Docker Compose integration for local development.
- GraalVM native images are possible, but should be treated as an optimization path, not the default beginner deployment.
- Production deployment must separate build, release, and run config.
...
bootRun locally
  ...
  -> managed platform/VPS
  -> Kubernetes only after operational need appears
```
