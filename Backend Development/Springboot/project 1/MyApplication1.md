 # MyApplication — Chapters 1 to 5 (Multi-Module Monolith)
...
```
myapplication-backend/          ← root project
...
CREATE USER myapplication WITH PASSWORD 'myapplication123';
CREATE DATABASE myapplication_dev OWNER myapplication;
GRANT ALL PRIVILEGES ON DATABASE myapplication_dev TO myapplication;
\q
```
...
```bash
psql -U myapplication -d myapplication_dev -h localhost
# Enter password: myapplication123
# Should show: myapplication_dev=#
# Type \q to exit
```
...
└── app/
    ...
    └── src/
        ├── main/
        │   ├── kotlin/com/myapplication/app/
        │   │   └── (empty for now)
        │   └── resources/
        │       └── application.yml
        └── test/kotlin/com/myapplication/app/
```

**On Mac/Linux — run this to create all folders at once:**

```bash
mkdir -p myapplication-backend/{core,user,content,notification,app}
cd myapplication-backend

for module in core user content notification; do
  mkdir -p $module/src/main/kotlin/com/myapplication/$module
  touch $module/build.gradle.kts
done

mkdir -p app/src/main/kotlin/com/myapplication/app
mkdir -p app/src/main/resources
mkdir -p app/src/test/kotlin/com/myapplication/app
...
```kotlin
// myapplication-backend/settings.gradle.kts
rootProject.name = "myapplication-backend"
...
```kotlin
// myapplication-backend/build.gradle.kts
plugins {
...
@SpringBootApplication(
    scanBasePackages = [
        ...
        "com.myapplication.notification"
...
> **Why `scanBasePackages`?** By default Spring only scans the package of `@SpringBootApplication` and its sub-packages. Your other modules live in different root packages (`com.myapplication.user`, etc.), so you must explicitly list them here.
...
Open IntelliJ IDEA → File → Open → select the `myapplication-backend` folder (not a subfolder).

IntelliJ detects the multi-module Gradle project automatically. You will see all 5 modules in the Project panel.

```bash
# Run from terminal inside myapplication-backend/
./gradlew :app:bootRun
```
...
```bash
./gradlew :app:bootRun
# Console should show: Started MyapplicationApplication in X.X seconds
```
...
- [ ] PostgreSQL running — `psql -U myapplication -d myapplication_dev -h localhost` connects
...
```
core/src/main/kotlin/com/myapplication/core/
├── exception/
│   ├── Exceptions.kt
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/exception/Exceptions.kt
package com.myapplication.core.exception
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/exception/GlobalExceptionHandler.kt
package com.myapplication.core.exception
...
> **Why is `GlobalExceptionHandler` in `core` and not `app`?** Because `@RestControllerAdvice` beans need to be in a package that Spring scans. You scan `com.myapplication.user`, `com.myapplication.content`, etc. — but you do NOT scan `com.myapplication.core`. To fix this, register `core` as a scan target in a different way — see the note in Chapter 5 where you configure `scanBasePackages`.
...
// app/src/main/kotlin/com/myapplication/app/exception/GlobalExceptionHandler.kt
// ← handler here (Spring bean, scanned by app module)
```

```kotlin
// app/src/main/kotlin/com/myapplication/app/exception/GlobalExceptionHandler.kt
package com.myapplication.app.exception

import com.myapplication.core.exception.*
import org.springframework.http.HttpStatus
import org.springframework.http.ResponseEntity
import org.springframework.validation.FieldError
import org.springframework.web.bind.MethodArgumentNotValidException
...
└── app/
    ...
    └── src/main/kotlin/com/myapplication/app/
        ├── MyapplicationApplication.kt     ✅ done
        └── exception/
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/enums/Difficulty.kt
package com.myapplication.core.enums
enum class Difficulty { EASY, MEDIUM, HARD }

// core/src/main/kotlin/com/myapplication/core/enums/Outcome.kt
package com.myapplication.core.enums
enum class Outcome { OFFER, REJECTED, GHOSTED }

// core/src/main/kotlin/com/myapplication/core/enums/SourceType.kt
package com.myapplication.core.enums
enum class SourceType { USER_SUBMITTED, CRAWLED }

// core/src/main/kotlin/com/myapplication/core/enums/InterviewStatus.kt
package com.myapplication.core.enums
enum class InterviewStatus { PENDING, PUBLISHED, REJECTED }

// core/src/main/kotlin/com/myapplication/core/enums/NotificationType.kt
package com.myapplication.core.enums
...
// core/src/main/kotlin/com/myapplication/core/enums/Platform.kt
package com.myapplication.core.enums
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/User.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/Company.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/Interview.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/InterviewRound.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/Tag.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/Bookmark.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/ReadHistory.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/Comment.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/Notification.kt
package com.myapplication.core.entity
...
```kotlin
// core/src/main/kotlin/com/myapplication/core/entity/FcmToken.kt
package com.myapplication.core.entity
...
- [ ] Connect to DB in IntelliJ (Database panel → + → PostgreSQL → host localhost, db myapplication_dev) → all tables visible
...
```
user/src/main/kotlin/com/myapplication/user/
├── repository/
│   └── UserRepository.kt
├── service/
│   └── UserService.kt
...
```kotlin
// user/src/main/kotlin/com/myapplication/user/repository/UserRepository.kt
package com.myapplication.user.repository

import com.myapplication.core.entity.User
...
```kotlin
// user/src/main/kotlin/com/myapplication/user/dto/UserDtos.kt
package com.myapplication.user.dto
...
```kotlin
// user/src/main/kotlin/com/myapplication/user/service/UserService.kt
package com.myapplication.user.service

import com.myapplication.core.entity.User
import com.myapplication.core.exception.ResourceNotFoundException
import com.myapplication.user.dto.*
import com.myapplication.user.repository.UserRepository
...
```kotlin
// user/src/main/kotlin/com/myapplication/user/controller/UserController.kt
...
```
app/src/main/kotlin/com/myapplication/app/
├── MyapplicationApplication.kt
├── exception/
...
```kotlin
// app/src/main/kotlin/com/myapplication/app/security/FirebaseJwtValidator.kt
package com.myapplication.app.security

import com.myapplication.user.dto.FirebaseUserInfo
...
```kotlin
// app/src/main/kotlin/com/myapplication/app/security/FirebaseJwtFilter.kt
package com.myapplication.app.security
...
```kotlin
// app/src/main/kotlin/com/myapplication/app/security/SecurityConfig.kt
package com.myapplication.app.security
...
```kotlin
// app/src/main/kotlin/com/myapplication/app/config/WebConfig.kt
...
4. Copy "Project ID" — looks like `myapplication-app-12345`
